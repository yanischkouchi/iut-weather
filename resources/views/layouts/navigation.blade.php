                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @foreach (auth()->user()->listCities as $city)
                        <div style="display: flex;">
                            ...
                            :active="request()->routeIs('weather.city') && request()->route('cityId') == $city->id">
                                {{ $city->city_name }}
                            </x-nav-link>
                            <form action="{{ route('weather.city.delete', $city->id) }}" method="POST" style="margin-top: 3.9vh; margin-left: 0.7vw; font-size: xx-small;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette ville ?')">❌</button>
                            </form>
                        </div>
                    @endforeach
